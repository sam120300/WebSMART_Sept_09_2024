...
class Login extends DBConnection {
	...
	function student_login(){
		extract($_POST);
		$qry = $this->conn->query("SELECT *,concat(lastname,', ',firstname,' ',middlename) as fullname from student_list where student_id = '$student_id' and `password` = md5('$password') ");
		if($this->conn->error){
			$resp['status'] = 'failed';
		...
		$resp['status'] = 'failed';
		$resp['msg'] = "Invalid student ID or password.";
		}
		}
