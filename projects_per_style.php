...
if(isset($_GET['id'])){
    ...
    $qry = $conn->query("SELECT * FROM reference_style WHERE id = '{$_GET['id']}' ");
    if($qry->num_rows > 0){
    ...
    <div class="col-12 mt-3">
        <div class="card card-outline card-success shadow rounded-0">
            <div class="card-body rounded-0">
                <h2>Archive List of <?= isset($reference_style['id']) ? $reference_style['style'] : "" ?> </h2>
                <hr class="bg-navy">
                <?php 
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            <ul class="pagination pagination-sm m-0 float-right">
                                <li class="page-item"><a class="page-link" href="./?page=projects_per_style&id=<?= $id ?>&p=<?= $page - 1 ?>" <?= $page == 1 ? 'disabled' : '' ?>>«</a></li>
                                <?php for($i = 1; $i<= $pages; $i++): ?>
                                <li class="page-item"><a class="page-link <?= $page == $i ? 'active' : '' ?>" href="./?page=projects_per_style&id=<?= $id ?>&p=<?= $i ?>"><?= $i ?></a></li>
                                <?php endfor; ?>
                                <li class="page-item"><a class="page-link" href="./?page=projects_per_style&id=<?= $id ?>&p=<?= $page + 1 ?>" <?= $page == $pages || $pages <= 1 ? 'disabled' : '' ?>>»</a></li>
                            </ul>
                        </div>
                    </div>
                </div>
